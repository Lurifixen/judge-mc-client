# Judge.se minecraft server infromation.
Vi är en vanilla whitelistad SMP server. Kontakta @_kribban eller @lurifixxen för information om hur man kan bli whitelistad.
Serverip: minecraft.judge.se:25566

## Klientmods.
Vi har fåtal [fabric loader](https://fabricmc.net/) mods installerade på serversidan som även du som spelare kan dra nytta utav. Dessa mods är dock **inte** obligatoriska utan du kan ansluta till servern helt utan mods om du hellre vill göra det.

* [Armor Poser](https://modrinth.com/mod/armor-poser) Används för att manipulera armorstands i olika poseringar. Shiftklicka på ett armorstand för att få upp menyn.
* [Simple Voice Chat](https://modrinth.com/plugin/simple-voice-chat) Gör det möjligt att snacka med varandra i spelet, fungerar även som grund för att applicera custom ljud på gethorn & musikdiskar.
* [Shulker Box Labels](https://modrinth.com/mod/shulker-box-labels) En mod som gör att du kan applicera items på dina shulkerboxar för att enkelt kunna se vad dom innehåller.
* [Inventory Sorting](https://modrinth.com/mod/inventory-sorting) Sortera din inventory med ett enkelt musklick. Klientbaserade mods för detta ändamål skapar onödigt lagg på servern så detta är ett bra alternativ.

# Modpack. 
[Judge MC Modpack](https://github.com/Lurifixen/judge-mc-client/blob/main/Judge1.21.10.1.mrpack)
Modpacket installeras via [Modrinth Launcher](https://modrinth.com/app)

## Mods i packet.
* [AmbientEnvironment](https://modrinth.com/mod/DyTvM1dv)
* [Axiom](https://modrinth.com/mod/N6n5dqoA)
* [BadOptimizations](https://modrinth.com/mod/g96Z4WVZ)
* [Better Clouds](https://modrinth.com/mod/5srFLIaK)
* [Camera Utils](https://modrinth.com/mod/rrwQMaWQ)
* [Chat Heads](https://modrinth.com/mod/Wb5oqrBJ)
* [Cloth Config v20](https://modrinth.com/mod/9s6osm5g)
* [Dynamic FPS](https://modrinth.com/mod/LQ3K71Q1)
* [EntityCulling](https://modrinth.com/mod/NNAgCjsB)
* [Fabric API](https://modrinth.com/mod/P7dR8mSH)
* [FerriteCore](https://modrinth.com/mod/uXXizFIs)
* [Freecam](https://modrinth.com/mod/XeEZ3fK2)
* [ImmediatelyFast](https://modrinth.com/mod/5ZwdcRci)
* [Inventory Sorter](https://modrinth.com/mod/5ibSyLAz)
* [Litematica](https://modrinth.com/mod/bEpr0Arc)
* [Lithium](https://modrinth.com/mod/gvQqBUqZ)
* [Locator Heads](https://modrinth.com/mod/L4pt5egz)
* [Locator Lodestones](https://modrinth.com/mod/pMBcsVIg)
* [MaLiLib](https://modrinth.com/mod/GcWjdA9I)
* [MiniHUD](https://modrinth.com/mod/UMxybHE8)
* [Mod Menu](https://modrinth.com/mod/mOgUt4GM)
* [Mouse Tweaks](https://modrinth.com/mod/aC3cM3Vq)
* [Placeholder API](https://modrinth.com/mod/eXts2L7r)
* [Presence Footsteps](https://modrinth.com/mod/rcTfTZr3)
* [Scribble](https://modrinth.com/mod/yXAvIk0x)
* [Shade & Saturation Slider](https://modrinth.com/mod/TjdLmzvL)
* [Shulker Box Labels](https://modrinth.com/mod/a4byiEVJ)
* [Shulker Box Tooltip](https://modrinth.com/mod/2M01OLQq)
* [Simple Fog Control](https://modrinth.com/mod/Glp1bwYc)
* [Smooth Skies](https://modrinth.com/mod/uvlgIwBD)
* [Sodium](https://modrinth.com/mod/AANobbMI)
* [Sound Physics Remastered](https://modrinth.com/mod/qyVF9oeo)
* [YOSBR](https://modrinth.com/mod/WwbubTsV)
* [YetAnotherConfigLib](https://modrinth.com/mod/1eAoo2KR)

## Datapacks
Server använder ett fåtal datapacks för att förbättra spelupplevelse. Dessa behöver du som spelar inte installera men vissa behöver aktiveras via ingame chatten för att fungera.

* Afk display. Ditt namn i blir grått i Tabmenyn för att indikera att du är AFK.
* Coordinates hud. Gör det möjligt att via chatten skriva "/trigger ch_toggle" för att ovanför din hotbar visa dina aktiva koordinater.
* Double shulker shells. Shulkers droppar två shulkerskal.
* Durability ping. När dina verktyg håller på att gå sönder så spelas ett ljud upp för att varna dig.
* Fast leaf decay. Löv droppar sina items mycket snabbare.
* Homes. Via chattkommandot /trigger sethome add 1 kan du sätta din hemposition. Med kommandot /trigger home kan du sen teleportera till ditt hem.
* More mob heads. Mobs har kan nu droppa deras huvuden.
* Player heads. Spelare har samma möjlighet att droppa deras huvuden.
* Name formating station. Med ett namngivet item i din hand kan du nu skriva /trigger FormatName i chatten för att få en lista där du kan modifiera hur texten visas för ditt item.
* Nether portal coords. Smidigt att räkna ut var i nether eller overworld din portal kommer hamna. /trigger nc_inOverworld för nether koordinater och /trigger nc_inNether för overworld.
* Spawn. /trigger spawn teleporterar dig till spawn.
* Wandering trades. Wandering traders säljer nu miniatyrblocks.
